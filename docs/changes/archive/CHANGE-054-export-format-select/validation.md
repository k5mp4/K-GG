...
- GIF最大ファイルサイズ追加後（Windows）: `npm run check:native` pass（Rust 43件。GIF縮小フィルター・最大サイズ検証・縮小倍率計算を含む）、`npx tsc -b` pass、`vitest src/adapters` pass（GIFのみ`gifMaxFileMb`を送り既定15MBとなることを含む）。
...
- Windows Tauri版（`npm run tauri:dev`）でGIF書き出しと最大ファイルサイズ指定の動作を利用者が確認した（2026-09-25）。
...
- 大きな解像度・長尺でのGIFが既定の15MB未満に収まること、縮小再エンコードを含むエンコード時間を記録する（Observation）。
