...
Diffuseの適応ソースは輝度、色相、彩度から選択できる。選択した値をX軸として拡散量Bezierを評価する。粒度適応を有効にした場合は独立した粒度Bezierをベースセル単位の代表色へ評価し、セル内座標を固定したままベース粒度へ反映する。これによりHalftoneの円形・四角形がフラグメント境界で崩れない。無効時は既存の輝度カーブと固定粒度の挙動を維持する。
