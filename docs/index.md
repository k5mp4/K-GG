...
## Canvas Size
出力解像度を設定します。
- **Full HD / HD / 400x400 / 800x800**: セレクトから一般的な解像度へ切り替えられます。初期値はFull HD（1920x1080）です。
- **W / H**: 直接数値を入力可能。マウスホイールで±1、Shift+ホイールで±10の調整が可能です。
- **南京錠アイコン**: アスペクト比を固定します。
  - 任意のアスペクト比に変更可能です。最大解像度は3840×3840pxです。

## Gradient Ramp
解像度の直下にある主要編集領域です。グラデーションの各ポイントの色や位置を調整できます。Ramp内の「Color Palette Generator」から、画像の色をストップとして抽出して適用できます。
...
## Image Overlay
折りたたみセクションから画像を重畳できます。
::: tip TIPS
既存のデザインにスリットの幅を調整する時に役立ちます。
:::
## Image Gradient Source
折りたたみセクションから、画像の輝度またはRGBチャンネルを現在のGradient Rampで再配色します。画像は中央基準のCoverで固定し、歪みはアンカー配色フィールドにのみ適用されます。
...
- **プロポーショナル移動**: カラーストップをShiftを押しながらドラッグすると、近くのストップほど強く追従し、離れたストップほど小さく追従します。ランプの端を越えることはありません。
# ウインドウパネル
## キャンバス操作
...
- `Perlin`
  - 3D Perlinノイズ（画面XY＋時間Z）から、暗い領域を柔らかく光る筋が横切るテクスチャ（AEフラクタルノイズの「にじみ」風）を作り、Direction方向へグラデーションを歪ませます。アニメーションでは形がその場で有機的に変化します。Roughnessで細部、Sharpnessで筋の細さ、Layer Mixで淡い副次的な筋の量を調整できます。
- `Phasor Lines`
...
- V2ではキャンバス左上の `Effect Stack` パネルで、Noise / Slit / Stretch / Distort / Mirror / Kaleidoscope / Voronoi / Glass / Glass V2 / Diffuse の順序を変更できます。Diffuseは初期状態では最後尾です。
...
- モジュールの表示順は `Surface → Main Stack → Prism → Particles` で固定され、SANDBOX内の選択変更は描画順を変更しません。SANDBOXの`Edit Layer`からCloth／Cone／Normal／Prism／Particlesを一つずつ選択します。
- SANDBOXのCloth／ConeモジュールをONにすると、処理済みCanvasをそれぞれの3D表示へ切り替えられます。専用のPreview Surface表示モードはなく、OFFにすると2D Canvasへ戻ります。準備中や利用できない環境ではCanvas表示を維持し、表示状態はPresetへ保存されません。
- ConeはDepth、Perspective、Rotation、Texture Repeat、Flow Cyclesを調整できます。照明を加えずCanvasの描画色を使い、Texture FlowはAnimationのタイムラインと静止画・動画書き出しへ同期します。Cone設定はPresetへ保存されます。
...
- プリセットはEffect Stackを反映した描画サムネイル付きのリスト/グリッドで表示できます。WebGLが使えない場合は軽量プレビューへ切り替わります。
- 保存時は選択中のフォルダが保存先になります。フォルダカードには配下の代表5件が表示されます。
- フォルダーツリーまたはフォルダーカードへプリセットをドラッグアンドドロップして、後から保存先を整理できます。階層は上段、ライブラリは下段に表示され、保存・書き出し・読み込みは下部に常時表示されます。
- 単一プリセットはJSON、フォルダまたはライブラリ全体はZIPで書き出し・読み込みできます。既存のフラットJSONも読み込めます。
- ブラウザ版はローカルストレージ、デスクトップ版はアプリデータ配下のプリセットファイルに保存されます。
