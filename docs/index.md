...
- V2ではキャンバス左上の `Effect Stack` パネルで、Noise / Slit / Stretch / Distort / Mirror / Kaleidoscope / Voronoi / Glass の順序を変更できます。Diffuseは画像系処理の最終段に固定されます。
- 行のグリップをドラッグすると、描画順序が上下に入れ替わります。
- 各行のスイッチで、その主スタックレイヤーのON/OFFを切り替えられます。Texture / Transform / Structure のカテゴリ表示は順序を制限しません。
- Normal/Matcapは主スタック前のSurface、PrismとDiffuseは主スタック後、Particlesは最終オーバーレイとして `Surface → Main Stack → Prism → Diffuse → Particles` の固定順で描画されます。
