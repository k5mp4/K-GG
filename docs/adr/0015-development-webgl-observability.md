...
Developmentビルドだけで生成するProfilerを既存WebGL contextへ任意参照として接続し、既存のdraw境界を観測する。Effect名は描画時のEffect Stack引数から取得する。GPU時間は`EXT_disjoint_timer_query_webgl2`の非同期queryを後続フレームで読む。timer-queryを使うK-GG固有Profilerは通常のDevelopment起動でも利用できる一方、stats-gl、webgl-memory、webgl-lintは`VITE_KGG_WEBGL_DEBUG_TOOLS=1`を指定したDevelopment起動だけでdynamic importする。これらのAPI検証ラッパーはValidationを無効化してもWebGL呼び出しを包むため、通常起動のCPU/FPS比較へ持ち込まない。Productionビルドは直接描画関数を選び、Profilerのquery・統計・Validation処理を実行しない。

Spector.jsはCapture操作時だけ既存Canvasを渡す。webgl-lintはcontext生成前に有効化できるが、Program情報との競合を避けるため、Validationを有効化したDevelopment環境ではKHR並列Shaderコンパイルを使わず同期リンクする。Validationが無効な通常起動では並列コンパイルを維持し、初期描画のメインスレッドブロックを避ける。並列完了通知がwatchdog期限内に返らない通常のlazy ShaderはProgramを破棄せず、最終compile/link status参照による同期確認へフォールバックし、実際のcompile/link失敗だけを失敗として扱う。Benchmark中はValidationを無効化する。lazy ShaderのUniform反映はProgram公開前に完了させ、単独の反映失敗でEffect Stack全体を停止させない。Profilerは保存形式、seed、time、frame index、texture、framebuffer stateを変更しない。
