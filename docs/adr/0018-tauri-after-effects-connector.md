...
7. Tauriの動画自動送信は通常Exportで確定したファイルを直接参照することを既定とし、AE専用フォルダーを明示した場合だけ送信先へコピーする。Browser版Bridgeと画像送信は既存の送信先処理を維持する。
8. After Effectsレイヤー情報はK-GGの`StoreSnapshot`へ直接混ぜず、連携専用DTOとして読み込む。K-GGへ変換できる設定だけを明示的にマッピングする。
