...
- macOSのDMGは現時点ではコード署名・notarizationを行わず、試験配布として明示する。
...
| macOSにもupdater artifactを生成する | 署名・公証・更新 endpointの設計が未確定で、未署名DMGの自動更新を約束できない |
| After Effects連携をmacOSへ移植する | OS間で異なるAfter Effects自動操作機構の設計・検証が別Requestになる |

## 結果

Windowsのリリース、FFmpeg探索、After Effects連携は変更せず、macOSはFFmpeg PATH、DMG、手動更新、未署名・未公証の試験配布という明示的な境界で提供する。macOS実機のGPU、WebView、DMG起動、Gatekeeper挙動はRelease Gateで確認する。
