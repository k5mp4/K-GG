...
# SPEC-013: Unified Effect Stack V2
...
- Noise、Slit、Stretch、Distort、Mirror、Kaleidoscope、Voronoi、Glass、Glass V2、Diffuseを各1つ、任意順で描画でき、Diffuseは既定で最後尾に配置される。
...
- 10種類の順序可変スタック、カテゴリ表示、選択中効果の詳細編集
...
`EffectPipelineConfig`を永続状態へ追加する。`version`は`legacy-v1`または`stack-v2`、`effectStack`は10種類の順序可変レイヤーの一意なレコード、`selectedKind`は詳細編集対象、Prism/Particlesは個別の有効状態を持つ。Diffuseは既定順で最後尾に置く。V2の有効状態はレイヤーだけを一次情報とし、既存詳細設定の重複した`enabled`はLegacy互換専用とする。

V2はベース画像を生成した後、主スタックの各層を入力textureから出力textureへ順に描画する。PrismとParticlesは主スタック外の固定段として描画し、Diffuseは主スタック内の指定位置で一度だけ適用する。既定順ではDiffuseが最後尾となる。GlassとGlass V2は主スタックに含み、`glassNoiseInfluence`は主スタックのNoiseの有効状態・位置に依存しないGlass専用入力として残す。NormalはDiffuseの有効状態に依存しない。

新規V2の並べ替え可能な既定順は`Noise -> Slit -> Stretch -> Distort -> Mirror -> Kaleidoscope -> Voronoi -> Glass -> Glass V2`、初期有効効果は固定最終段のDiffuseだけとする。V2のDiffuseは既存`DiffuseConfig`とDiffuseパネルのアルゴリズムを唯一の設定・描画源とし、旧Postprocess Diffuseは使わない。規則的な格子が目立たないよう、Diffuseの乱数場は決定的かつタイル連続な低周波ワープを加える。Diffuseのサンプル座標は前段NoiseのUVを引き継がず、キャンバスのグローバル座標から独立に計算する。BlockモードではNoiseが前段にあっても、セル配置がNoise UVによって歪まない。Diffuse直後にSlitがある場合はSlitの出力座標側でDiffuseの変位を評価し、Slitの延長領域でもセルが引き延ばされないようにする。VoronoiはRampだけを再生成せず、直前のスタック入力テクスチャ全体を各Voronoiセルへタイル配置し、その入力画像自体をVoronoi状に変換する。Distortは既存の生成側`ManualDistortConfig`を使う。
...
Noise、Slit、Distort、Mirror、Kaleidoscope、Voronoi、固定Diffuse、最終copyは軽量stack coreシェーダーで描画し、巨大な統合シェーダーへ依存しない。Distortは変位map sampling、KaleidoscopeはUV変換、Voronoiはセル生成と直前入力テクスチャのタイル配置だけをcoreへ含め、元のGradient Rampを色源として重ねず、GlassやPrismの関数を参照しない。
...
- AC-001: 10種類を1本のEffect Stackで表示して任意順へドラッグでき、Diffuseは既定で有効・最後尾に配置される。
...
- AC-007: GlassまたはGlass V2を含む任意順のV2スタックはタイル出力で必要なガターを確保する。
...
| AC-011 | 直接描画・FBO allocation mode・core/Glass/Glass V2/Prism program境界のテストと、GPU初期化後の表示確認 | `effectPipeline.test.ts`、`effectShaderParity.test.ts`、WebGLプレビューの手動確認 |
| AC-012 | program要求分類テスト、Glass/Glass V2/Prism個別コンパイル、実WebGL描画 | `effectPipeline.test.ts`、WebGLプレビュー |
