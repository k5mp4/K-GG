...
専用programのソース組み立てでは、別パスで適用済みのDiffuse実装全体を含めず、Glassが参照する
`diffusePanelDisplacement`と`applyDiffuseDither`の恒等スタブを明示的に挿入する。
...
- AC-011: GlassとGlass V2の専用fragmentにはDiffuseの恒等スタブを明示的に含め、Diffuse本体の関数群を含めない。
...
| AC-011 | Glass専用fragmentのソース組み立て契約テスト | `webglShaderSources.test.ts` |
