...
| SPEC-036 | [Tweeqカーブ・シャッフル入力への統一](./SPEC-036-tweeq-curve-shuffle-controls.md) | implemented |
...
既存の `SPEC-000`〜`SPEC-040` は、公開済みリンクと履歴追跡性を維持するため直下に残しています。新しい変更には `CHANGE-YYYYMMDD-slug` を使い、既存SPEC IDを再利用しません。現在の動作と過去の意図が異なる場合は、[DocDD運用ガイド](../development/docdd#仕様と実装が異なる場合)に従って差異を報告します。
