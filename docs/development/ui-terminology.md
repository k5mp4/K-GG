...
| Effect Stack / Workspace | `effect.*`, `workspace.*`, `stack.swapHistogram`, `stack.version`, `stack.popOut`, `stack.fixed`, `stack.drag`, `stack.status.*`, `histogram.*`, `postprocess.glass.*` |
