...
const siteData = deserializeFunctions(JSON.parse('{"lang":"en-US","dir":"ltr","title":"K-GG","description":"A professional gradient generator tool for creators.","base":"/docs/","head":[],"router":{"prefetchLinks":true},"appearance":true,"themeConfig":{"nav":[{"text":"Home","link":"/"}],"sidebar":[],"outline":{"level":[1,3],"label":"How to use this App"}},"locales":{},"scrollOffset":134,"cleanUrls":false}'));
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const cloudinaryUrl = computed(() => {
      const transforms = `f_auto,q_auto${props.width ? `,w_${props.width}` : ""}${props.height ? `,h_${props.height}` : ""}`;
      return `https://res.cloudinary.com/${cloudName}/image/upload/${transforms}/${props.publicId}`;
    });
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<img${ssrRenderAttrs(mergeProps({
        src: cloudinaryUrl.value,
        alt: __props.alt
      }, _ctx.$attrs, { loading: "lazy" }, _attrs))}>`);
    };
  }
};
const _sfc_setup = _sfc_main.setup;
_sfc_main.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add(".vitepress/theme/components/CldImage.vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};
const RawTheme = {
  extends: theme,
  enhanceApp({ app }) {
    app.component("CldImage", _sfc_main);
  }
};
