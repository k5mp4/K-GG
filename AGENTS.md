...
- ADRは`npm run adr:new -- <slug> --title="..."`で作り、IDは`ADR-YYYYMMDD-slug`（ファイル名`YYYYMMDD-slug.md`）を使う。`ADR-0001`〜`ADR-0023`は履歴用で、新しい連番を採番しない。
- 並列PRで衝突しないよう、Change一覧・ADR一覧のindex.mdへ行を追記せず、Current Specの`related_changes`へも追記しない（一覧と逆参照はfrontmatterから生成する）。
