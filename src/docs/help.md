...
- **Image Gradient Source**: 読み込んだラスタ画像のLuminance / Red / Green / Blueを現在のGradient Rampで再配色します。画像はCoverで配置され、画像本体はプリセットへ保存されません。
...
- キャンバス左上の `Effect Stack` パネルで、Noise / Slit / Stretch / Distort / Mirror / Kaleidoscope / Voronoi / Glass の順序を変更できます。
- 行のグリップをドラッグすると描画順序が入れ替わり、各行のスイッチでレイヤーをON/OFFできます。
- DiffuseはPrism後・Particles前の最終画像処理として固定され、Diffuseパネルで設定します。
- 固定順は `Surface → Main Stack → Prism → Diffuse → Particles` です。
