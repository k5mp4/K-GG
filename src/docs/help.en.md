...
Use the Effect Stack panel at the upper-left of the canvas to reorder Noise, Slit, Stretch, Distort, Mirror, Kaleidoscope, Voronoi, Glass, GlassTile, Video Motion, Cone, and Diffuse. Drag a row by its grip and toggle it with the switch. Postprocess is shown as enabled whenever one of Stretch, Distort, Mirror, Kaleidoscope, Voronoi, Glass, GlassTile, Video Motion, or Cone is enabled in the Effect Stack. Edit hand-drawn `Distort` from Postprocess's `Edit Layer`; old `manualDistort` preset data is migrated there on load. The fixed stages are `Surface → Main Stack → Prism → Particles`; Cone is a normal Main Stack layer, while Normal, Prism, and Particles are edited from the `SANDBOX` tab in the top bar.
...
The top bar is ordered `Diffuse → Noise → Slit → Postprocess → SANDBOX → Export → Preset`; Stretch has no standalone tab and is not shown in the Postprocess property module. Select an Edit Layer in Postprocess to operate that layer's detailed properties. SANDBOX uses the same text color as Postprocess. The `SANDBOX` top-bar tab groups drawing modules outside the reorderable gradient stack. Use its `Edit Layer` selector to edit Cloth, Normal, Prism, Particles, Flow Gradient, or Seamless; changing the selection does not change the render order. Cone is managed as a normal Effect Stack layer.
...
Cone appears as a normal Effect Stack layer with the same enable, drag, randomize, and solo controls as other layers. Its GPU pass maps the preceding stack texture onto the cone surface, preserves that texture's colors, and sends the result to downstream layers. Drag the single cyan Cone apex handle beyond the canvas edge; its normalized position is limited to -2..2. There is no auxiliary ring, crosshair, or inner marker. Use Reset Position to return it to the center. The shared gradient-anchor visibility control also hides the Cone handle. Seam Mode is always labeled `Mirror Repeat`, `Edge Weld`, or `Gradient Reapply`, regardless of language. Seam Blend makes texture-repeat and flow seams continuous; Gradient Reapply corrects RGB while preserving the center sample's alpha. Cone settings, enabled state, and layer order are saved in presets.
The Cone pass uses the main WebGL stack renderer. If WebGL2 is unavailable in the browser or WebView, Cone cannot be drawn and the normal 2D Canvas remains available for editing. Reload the page to re-detect WebGL2 after changing the environment.

Cone properties include Mapping (Flow / Direct Projection), Depth (up to 30), Rotation, Texture Repeat, Seam Mode, Seam Blend, and Flow Cycles (±30). Perspective is not exposed. Mirror Repeat is the default Seam Mode. Direct Projection fixes the V offset; Flow follows the shared Animation timeline and export frames. Edit Gradient Ramp in the right sidebar: it colors the preceding texture, which Cone projects without replacing it with a separate ramp. Apex editing remains on the canvas, and no environment or surface lighting is added.
